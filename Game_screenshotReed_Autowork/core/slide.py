...
class Slide:
	...
	def __init__(self, adb_path: str | None = None, default_instance: int = 1) -> None:
		# MuMu 默认 adb 路径; 可传入自定义路径覆盖
	...
	def _port_for(self, instance_num: int | None) -> int:
		base = self.default_instance if instance_num is None else instance_num
		return 7554 + base

	def _run_adb(self, args: str, instance_num: int | None = None) -> None:
		port = self._port_for(instance_num)
		cmd = f'"{self.adb_path}" -s 127.0.0.1:{port} {args}'
